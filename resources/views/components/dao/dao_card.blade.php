<div class="flex flex-col rounded-lg shadow-lg overflow-hidden">
    <div class="flex items-center justify-center flex-col relative h-48 bg-theme-dark text-white">
        <div class="text-4xl font-bold">{{ $dao->name ?? '' }}</div>
        <div>{{ $dao->type ?? '' }}</div>
        <a href="{{ route('show_dao', ['dao_id' => $dao->id]) }}" class="absolute -bottom-4 right-4 "><button type="button"
        ...
        <div class="mt-6 flex items-center">
            ...
            <div class="ml-3">
                <p class="text-sm font-medium text-gray-900">
                    <a href="#" class="hover:underline"> Dante Velli </a>
                </p>
