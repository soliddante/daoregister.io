<div>
    ...
    @endif
    <div class="mt-1">
        <input role="presentation" onfocus="this.removeAttribute('readonly');"  {!! isset($disabled) ? 'disabled' : '' !!} {!! isset($empty) ? 'readonly' : '' !!} autocomplete="nope" value="{{ $value ?? '' }}" type="{{ $type ?? 'text' }}" name="{{ $name ?? '' }}" id="{{ $id ?? '' }}"
